...
const ItemDetail = ({uniObjs}) => {
  const { id } = useParams();
  const universeObjFilter = uniObjs.filter ((universeObj) => universeObj.id == id);
          ...
          <Card bgGradient='linear(to-r, green.200, pink.500)' className="card-main">
            <CardBody>
              <Stack mt="6" spacing="3">
                <Center>
              <Image borderRadius="lg" width='60%' height='40%' src={universeObj.image} />
              </Center>
            ...
            <CardFooter className="card-footer">
              <ItemCount top={(universeObj.stock)}/>
            </CardFooter>
          </Card>
        </Center>
      </div>
